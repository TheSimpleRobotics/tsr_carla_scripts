...
class CameraTsr(object):
    ...
    def __init__(self, vehicle, width, height, fov='90', tick='0.0'):
    ...
    def add_camera(self, x=0.8, y=0.0, z=1.7, 
        ...
        sensor = self.vehicle.get_world().spawn_actor(blueprint, transform, attach_to=self.vehicle)
...
class World(object):
    ...
    def restart(self):
        ...
        while self.player is None:
            ...
            self.player = self.world.try_spawn_actor(vehicle_bp, spawn_point)
        # Set up tsr cameras
        self.camera_tsr= CameraTsr(vehicle=self.player, width=str(320), height=str(200), fov="90")
        # Svs camera inputs
        self.cam1_svs_front = self.camera_tsr.add_camera(x=0.8, y=0.0, z=1.7, 
                                                         roll=0, pitch=-15, yaw=0,
                                                         camera_type='sensor.camera.rgb')

        self.cam2_svs_right = self.camera_tsr.add_camera( x=0.0, y=0.5, z=1.7, 
                                                         roll=0, pitch=-15, yaw=90,
                                                         camera_type='sensor.camera.rgb')

        self.cam3_svs_back = self.camera_tsr.add_camera(x=-0.8, y=0.0, z=1.7, 
                                                         roll=0, pitch=-15, yaw=180,
                                                         camera_type='sensor.camera.rgb')

        self.cam4_svs_left = self.camera_tsr.add_camera( x=0.0, y=-0.5, z=1.7, 
                                                         roll=0, pitch=-15, yaw=270,
                                                         camera_type='sensor.camera.rgb')
        # Set up the sensors.
